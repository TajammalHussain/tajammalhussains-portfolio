...
export async function withCache<T>(
  ...
  await env.CACHE_KV.put(cacheKey, JSON.stringify(data), { expirationTtl: ttlSeconds });
...
export async function checkRateLimit(env: Env, request: Request): Promise<boolean> {
  try {
    ...
    await env.CACHE_KV.put(key, String(current + 1), { expirationTtl: RATE_LIMIT_WINDOW_SECONDS + 5 });
    return true;
  } catch {
