...
describe("transformIntensityToSilver", () => {
  ...
  it("rejects a payload with no data array", () => {
    expect(() => transformIntensityToSilver({} as never)).toThrow(
      InvalidPayloadError,
    );
  });
  ...
  it("rejects an entry missing 'from'/'to'", () => {
    ...
    expect(() => transformIntensityToSilver(malformed as never)).toThrow(
      InvalidPayloadError,
    );
  });
      ...
      data: [
        {
          from: "a",
          to: "b",
          intensity: { forecast: "oops", actual: 40, index: "low" },
        },
      ],
    };
    expect(() => transformIntensityToSilver(malformed as never)).toThrow(
      ...
      data: [
        {
          from: "a",
          to: "b",
          intensity: { forecast: 50, actual: 40, index: "extreme" },
        },
      ],
    };
    expect(() => transformIntensityToSilver(malformed as never)).toThrow(
...
describe("transformGenerationToSilver", () => {
  ...
  it("rejects a payload missing generationmix", () => {
    expect(() => transformGenerationToSilver({ data: {} } as never)).toThrow(
      InvalidPayloadError,
    );
  });
  ...
  it("rejects a payload whose percentages don't sum to ~100", () => {
    ...
    expect(() => transformGenerationToSilver(malformed)).toThrow(
      InvalidPayloadError,
    );
  });
...
describe("transformGenerationRangeToSilver", () => {
  it("flattens multiple periods into one silver row list", () => {
    ...
    expect(rows.map((r) => r.periodFrom)).toEqual([
      "2026-01-01T00:00Z",
      "2026-01-01T00:30Z",
    ]);
  });
  ...
  it("rejects a payload without a data array", () => {
    expect(() => transformGenerationRangeToSilver({} as never)).toThrow(
      InvalidPayloadError,
    );
  });
});
